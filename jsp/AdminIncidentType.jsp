...
<jsp:useBean id="_recordBean" class="com.beans.AdminIncidentType" scope="request" />
<%
	...
	if(submitType == <%=SubmitType.CREATE_CONFIRM%>) {
		if(isValid) {
			...
			for(var i=0; i<numOfRows; i++) {
				if(i == 0) {
					mandatoryFieldsArray[mandatoryFieldsArray.length] = new Array(document.formmain["typeName"+i], "Type");
				}
			}
		...
		if(isValid) {
			var mandatoryFieldsArray = new Array();
			mandatoryFieldsArray[mandatoryFieldsArray.length] = new Array(document.formmain["typeName"], "Type");
			isValid = validateMandatoryFieldsInForm(mandatoryFieldsArray, isValid);
		}
...
<div class='row my-2'>
	<div class='col-12 mb-2'>
		<div class='card'>
			...
			<input type="hidden" id="incidentTypeID" name="incidentTypeID" value="<%=_recordBean.getIncidentTypeID()%>">
			<%if(submitType == SubmitType.CREATE) {%>
				<input type="hidden" id="dynamicParams" name="dynamicParams" value="typeName">
				<div class="row">
							...
							<div class='card-body m-1 p-1'>
								<table width="100%" border="0" cellpadding="0" cellspacing="0" class="table table-bordered table-striped table-hover table-sm table-block table-vertical sortable mb-0">
									...
									<thead class="thead-block">
										<tr class="tr-block">
											<th class="th-block table-header-label text-center required" width="100%">Type</th>
										</tr>
									</thead>
									<tbody class="tbody-block">
										<%for(int i=0; i<10; i++) {%>
										<tr class="tr-block">
											<td class="td-block table-value" data-th="Type"><textarea id="typeName<%=i%>" name="typeName<%=i%>" class="form-control form-control-sm" rows="1"></textarea></td>
										</tr>
										<%}%>
									</tbody>
								</table>
				...
				<div class="row">
					<div class="col-12">
						<div class="row form-row form-group form-group-sm">
							<label class="col-12 col-3 col-form-label text-left">Type</label>
							<div class="col-12 col-md-9 text-left"><textarea id="typeName" name="typeName" class="form-control form-control-sm"><%=_recordBean.getTypeName()%></textarea></div>
						</div>
					</div>
				</div>
				...
				<div class="row">
					<div class="col-12 col-md-5">
						<div class="row">
							...
							<div class="col-7 col-md-9 form-control-plaintext text-left"><%=_recordBean.getTypeName()%></div>
						</div>
					</div>
				</div>
			<%}%>
		...
		<%if(submitType == SubmitType.CREATE || submitType == SubmitType.UPDATE) {%>
			<div class="row mt-4">
				...
				<div class="col-4 text-center">
					<%if(submitType == SubmitType.CREATE) {%>
						<button class="btn btn-success text-center" onClick="Javascript:submitPageDataForm('<%=SubmitType.CREATE_CONFIRM%>','<%=_recordBean.getController()%>','<%=_recordBean.getIncidentTypeID()%>');">Save</button>
					<%} else {%>
						<button class="btn btn-success text-center" onClick="Javascript:submitPageDataForm('<%=SubmitType.UPDATE_CONFIRM%>','<%=_recordBean.getController()%>','<%=_recordBean.getIncidentTypeID()%>');">Save</button>
					<%}%>
				</div>
		...
		<%} else if(submitType == SubmitType.BROWSE) {%>
			<div class="row mt-4">
				...
				<%if("0".equalsIgnoreCase(_recordBean.getStatus())) {%>
					<button class="btn btn-primary text-center" onClick="Javascript:submitPageDataForm('<%=SubmitType.UPDATE%>','<%=_recordBean.getController()%>','<%=_recordBean.getIncidentTypeID()%>');">Edit</button>
				<%}%>
				</div>
				<div class="col-4 text-right">
				<%if("0".equalsIgnoreCase(_recordBean.getStatus())) {%>
					<button class="btn btn-danger text-center" onClick="Javascript:submitPageDataForm('<%=SubmitType.DELETE%>','<%=_recordBean.getController()%>','<%=_recordBean.getIncidentTypeID()%>');">Delete</button>
				<%}%>
				</div>
			</div>
		<%}%>
