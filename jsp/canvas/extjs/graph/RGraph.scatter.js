        ...
        */
        ...
        for (var j=0; j<this.data[i].length; ++j) {
            ...
            */
			...
			var tickmark  = data_point[j][3] ? data_point[j][3] : default_tickmark;
            var tooltip = (data_point[j] && data_point[j][4]) ? data_point[j][4] : null;
    ...
    */
    RGraph.Scatter.prototype.DrawMark = function (data_set_index, x, y, xMax, yMax, color, tooltip, coords, data, data_index, tickmarks)
    {
        ...
        */
        if (this.properties['chart.line.visible'] && typeof(y) == 'number' && !y0 && !y1 && !y2 && !y3 && !y4) {
			...
			} else if (tickmarks == 'filledcircle') {
                this.context.arc(x, yCoord, halfTickSize, 0, 6.28, 0);
            ...
            */
			...
			/**
            * Triangle shape tickmarks
            */
			}else if (tickmarks == 'triangle') {
				this.context.moveTo(Math.round(x - halfTickSize), yCoord + halfTickSize);
                this.context.lineTo(Math.round(x), yCoord - halfTickSize);
                this.context.lineTo(Math.round(x + halfTickSize), yCoord + halfTickSize);
				this.context.fill();
				this.context.closePath();
				this.context.stroke();

			/**
            * Inverted Triangle shape tickmarks
            */
			}else if (tickmarks == 'invertedtriangle') {
				this.context.moveTo(x, yCoord - 5);
				this.context.lineTo(x - 3, yCoord - 10);
				this.context.lineTo(x + 3, yCoord - 10);
				this.context.fill();
				this.context.closePath();
				this.context.stroke();
